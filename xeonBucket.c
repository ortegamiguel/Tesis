...
__attribute__((target(mic))) int pivot(int *unarray, int izq, int der)
{
    int i;
...
__attribute__((target(mic))) void Quicksort(int *unarray, int izq, int der)
{
     int pivote;
     if(izq < der){
...
int main()
{
		int n, i, j, higher, size_bucket, smaller;
		int *arr;
    ...
    #pragma offload_transfer target(mic:0) mandatory \
    nocopy(buckets:length(T) alloc_if(0) free_if(0))\
    nocopy(arr:length(n) alloc_if(0) free_if(0))
